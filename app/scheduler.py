...
from app.analysis.reddit_collector import collect_reddit_data, cleanup_deleted_posts
...
def _cleanup_job():
    """Remove deleted/removed Reddit posts and purge stale data (Developer Terms compliance)."""
    logger.info("[Scheduler] Starting Reddit data cleanup job…")
    db = SessionLocal()
    try:
        stats = cleanup_deleted_posts(db)
        logger.info(f"[Scheduler] Cleanup done: {stats}")
    except Exception as exc:
        logger.error(f"[Scheduler] Cleanup job failed: {exc}", exc_info=True)
    finally:
        db.close()
...
def create_scheduler() -> BackgroundScheduler:
    ...
    scheduler.add_job(_cleanup_job, "interval", minutes=CLEANUP_INTERVAL, id="cleanup",
